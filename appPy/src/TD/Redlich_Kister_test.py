...
class Redlich_Kister_test(Area):
    ...
    def __init__(self, compound1, compound2, dataset_name):
        ...
        self.keys_to_serialize = [
            'D', 'is_consistent', 'curve_dif', 'curve_sum', 'x_1', 'curve', 'D_criterion', 'is_isobaric'
        ]

        # the test criterion D [%]
        self.D_criterion = cfg.rk_D_criterion if self.is_isobaric else cfg.rk_D_criterion_isotherm
    ...
    def report(self):
        ...
        if self.is_consistent:
            ok_echo(f'D < {self.D_criterion:.0f}')
            ok_echo('OK, data consistency is proven')
        else:
            err_echo(f'D > {self.D_criterion:.0f}')
