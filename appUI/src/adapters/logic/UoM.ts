...
export type TemperatureUnitType = 'K' | '°C' | '°F';
...
export const display_T = (T: number, UoM: TemperatureUnitType) => {
    if (UoM === '°F') return K2F(T);
    if (UoM === '°C') return K2C(T);
    return T; // defaulting to K
};
// Convert temperature input from specified UoM to K.
export const input_T = (T: number, UoM: TemperatureUnitType) => {
    if (UoM === '°F') return F2K(T);
    if (UoM === '°C') return C2K(T);
    return T; // defaulting to K
};
// Convert vectorized temperature from K to specified UoM for display.
export const display_T_vec = (T_vec: number[], UoM: TemperatureUnitType) => T_vec.map((T) => display_T(T, UoM));
// Convert vectorized temperature input from specified UoM to K.
export const input_T_vec = (T_vec: number[], UoM: TemperatureUnitType) => T_vec.map((T) => input_T(T, UoM));
...
export type PressureUnitType = keyof typeof p_units;

// Convert pressure from kPa to specified UoM for display.
export const display_p = (p: number, UoM: PressureUnitType) => p * p_units[UoM as PressureUnitType];
// Convert pressure input from specified UoM to kPa.
export const input_p = (p: number, UoM: PressureUnitType) => p / p_units[UoM as PressureUnitType];

// Convert vectorized pressure from kPa to specified UoM for display.
export const display_p_vec = (p_vec: number[], UoM: PressureUnitType) => p_vec.map((p) => display_p(p, UoM));
// Convert vectorized pressure input from specified UoM to kPa.
export const input_p_vec = (p_vec: number[], UoM: PressureUnitType) => p_vec.map((p) => input_p(p, UoM));
