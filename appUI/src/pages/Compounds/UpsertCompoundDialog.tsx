...
export const UpsertCompoundDialog: FC<UpsertCompoundDialogProps> = ({ origCompound, open, handleClose }) => {
    ...
    const handleToggleFitting = useCallback(() => setFittingOpen((prev) => !prev), []);
                ...
                <Stack direction="column" gap={2} pt={1}>
                    ...
                    {modelDef && (
                        <Box mt={3}>
                            ...
                            <p>
                                Either fill in known values, <b>or</b> leave initial estimates and perform FITTING using
                                measured <i>p, T</i> data
                                <br />
                            ...
                            />
                            <Box>{!isDataWhole && <ErrorLabel title="Data is incomplete!" />}</Box>
                            <Button
                                onClick={handleToggleFitting}
                                variant="outlined"
                                disabled={isError}
                                startIcon={fittingOpen ? <KeyboardArrowUp /> : <KeyboardArrowDown />}
                            >
                                Fitting
                            </Button>
                            {fittingOpen && (
